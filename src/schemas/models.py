...
from aiofauna_llm import LLMStack
...
class FileData(FaunaModel):
    ...
    content_type: str = Field(..., description="The content type of the file.", index=True)
    last_modified: str = Field(default_factory=lambda: datetime.now().isoformat(), description="The last modified date of the file.", index=True)
...
class DataVisualization(FaunaModel):
    ...
    datasets: List[List[Union[str, float]]] = Field(default_factory=list)
    title: str = Field(..., description="The title of the data visualization.", index=True)
...
class BlogPost(FaunaModel):
    ...
    cover: Optional[str] = Field(default=None, description="The cover of the blog post.")
    description: str = Field(..., description="The description of the blog post.")
    theme: str = Field(default="light", description="The theme of the blog post.", index=True)
    category: str = Field(default="general", description="The category of the blog post.", index=True)
...
class BookOrDocument(FaunaModel):
    ...
    wordcloud: List[WordCount] = Field(..., description="The wordcloud of the document.", index=True)
    file:Optional[FileData] = Field(default=None, description="The file of the document.", index=True)
    audio: Optional[str] = Field(default=None, description="The audio of the document.", unique=True)
    cover: Optional[str] = Field(default=None, description="The image of the document.", unique=True)
