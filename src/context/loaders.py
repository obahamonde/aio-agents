...
async def csv_loader(
    file: FileField,
) -> AsyncGenerator[Dict[str, Union[str, List]], None]:
    """Reads a CSV file from the request and returns a list of strings"""
    data = file.file.read()
    reader = list(csv.reader(io.StringIO(data.decode("utf-8"))))
    chart_labels = reader[0][0].split(";")
    try:
        chart_data = [reader[1:][i][0].split(";") for i in range(len(reader[1:]))]
        for chart in chart_data:
            for index, value in enumerate(chart):
                try:
                    chart[index] = float(value)
                except ValueError:
                    chart[index] = value
        yield {"labels": chart_labels, "datasets": chart_data}
    except IndexError:
        pass
